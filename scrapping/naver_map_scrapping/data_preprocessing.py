...
#########   1. 데이터 전처리 후 저장할 파일 이름을 지정해주세요 저장시: naver(검색어).csv로 저장됩니다

query = "은평구맛집"

############   2. 불러올 csv 파일 이름을 입력해주세요

df = pd.read_csv("naver(은평구 맛집).csv")["data"]  # 파싱한 csv파일 불러오기
title = pd.read_csv("naver(은평구 맛집).csv")["title"]  # 파싱한 csv파일 불러오기
